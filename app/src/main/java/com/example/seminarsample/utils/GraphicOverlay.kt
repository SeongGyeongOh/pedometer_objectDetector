...
open class GraphicOverlay(context: Context?, attrs: AttributeSet?) :
    ...
    abstract class Graphic(private val overlay: GraphicOverlay) {
        ...
        fun calculateRect(height: Float, width: Float, boundingBoxT: Rect): RectF {
            ...
            val scale = scaleX.coerceAtLeast(scaleY) // coerceAtLeast : 최대값.coerceAtLeast(숫자)
    ...
    private fun initProcessCanvas () {
        processBitmap = Bitmap.createBitmap(measuredWidth, measuredHeight, Bitmap.Config.ARGB_8888) // Bitmap.Config.ARGB_8888 : 하나의 픽셀을 4byte를 이용해 색을 표혐
